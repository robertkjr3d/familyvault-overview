...
return removeHouseholdMember({ data: { householdId: selectedHouseholdId, email } });
},
...
{realDateLabel}
        ...
        {/* DEMO badge — clearly visible when viewing Demo Household */}
        {selectedHousehold?.name === "Demo Household — FamilyHub SG" && (
          <span className="shrink-0 rounded-full bg-review px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-review-foreground">
            DEMO
          </span>
        )}
              ...
              <SelectContent>
                {households.map((h) => (
                  <SelectItem key={h.id} value={h.id}>
                    {h.name === "Demo Household — FamilyHub SG" ? "🎭 Demo" : h.name}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            {people.members.map((m) => (
              <div key={m.email} className="flex items-center justify-between gap-2 text-xs">
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <span className="rounded-full bg-accent px-2 py-0.5 text-[10px] font-medium capitalize text-accent-foreground">
                    {m.role}
                  </span>
                  {!m.isYou && canShareActiveHousehold && (
                    <button
                      className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-urgent hover:bg-urgent/10 disabled:opacity-40"
                      disabled={removeMemberMutation.isPending}
                      onPointerDown={(e) => {
                        e.preventDefault();
                        if (!confirm(`Remove ${m.email}'s access to this household?`)) return;
                        removeMemberMutation.mutate({ email: m.email });
                      }}
                    >
                      Remove
                    </button>
