...
const { checked, toggle } = useEstateChecklist();
const [open, setOpen] = useState(false);

const doneCount = ESTATE_SECTIONS.flatMap((s) => s.items).filter((i) => checked.has(i.id)).length;
const total = ESTATE_SECTIONS.flatMap((s) => s.items).length;
...
```
  ...
  {open && (
    <ul className="mt-3 space-y-2">
      {ESTATE_SECTIONS.flatMap((s) => s.items).map((item) => {
        const done = checked.has(item.id);
        return (
          <li key={item.id} className={`rounded-xl border p-3 ${"urgent" in item && item.urgent && !done ? "border-urgent/30 bg-urgent-soft/10" : "border-border/60 bg-background/50"}`}>
            <button
              type="button"
