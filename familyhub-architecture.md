...
- **Two adviser-facing views (`advisor_client_summary_view`, `advisor_networth_components_view`) — Supabase flags both red/"Unrestricted." STILL OPEN as of 23 Sep 2026.** Confirmed via real diagnostic: both grant SELECT to literally every role, including fully unauthenticated visitors. Whether this is actually exploitable comes down to one specific database function (`has_advisor_access`) correctly saying "no" to an anonymous request — not yet confirmed either way, so treat as a real possibility, not a false alarm. **Next session opens with:** reading that one function's definition (a safe, read-only check) before deciding anything. **In plain terms either way:** fixing this does NOT reduce what a real adviser can see — the same permission check used everywhere else in the app just moves somewhere harder to bypass.
